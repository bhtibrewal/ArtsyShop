...
export const TotalBill = ({
  ...
  // apply coupon
  const applyCouponHandler = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (coupon === "ARTSY") {
  ...
  const priceDetails = {
    price: cart?.reduce(
      (acc: number, { qty = 0, original_price }: Product) =>
        acc + original_price * qty,
      0
    ),
    discount: cart?.reduce(
      (acc: number, { qty = 0, original_price, price }: Product) =>
        acc + (original_price - price) * qty,
      0
    ),
    shipping:
      cart?.reduce(
        (acc: number, { shipping = 0 }: Product) => acc + shipping,
        0
      ) || 100,
  };
  ...
  const makePayment = async (amount: number) => {
    ...
    const options = {
      ...
      handler: async function (response: { razorpay_payment_id: number }) {
    ...
    <section className="cart-price-sec">
      ...
      <form onSubmit={applyCouponHandler}>
        ...
        <InputField
          ...
          value={coupon}
          // type assertion
          onChange={(e) => setCoupon((e.target as HTMLInputElement).value)}
        />
      ...
      <ButtonPrimary
        onClick={onClickFunction ? () => onClickFunction() : placeOrderHandler}
      >
        <>
